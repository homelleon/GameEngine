...
public class Chunk {
	...
	public boolean isBlockExist(int x, int y, int z) {
		...
		if (x >= 0 && y >= 0 && z >= 0 && x <= EngineSettings.VOXEL_CHUNK_SIZE && y <= EngineSettings.VOXEL_CHUNK_SIZE
	...
	public boolean isBlockExist(Vector3i position) {
		boolean isExist = false;
		if (position.x >= 0 && position.y >= 0 && position.z >= 0 && position.x <= EngineSettings.VOXEL_CHUNK_SIZE
