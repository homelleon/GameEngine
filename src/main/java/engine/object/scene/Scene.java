...
public class Scene extends AObjectManager implements IScene {
	...
	private void initialize(IObjectManager levelMap) {
		...
		Vector3fF voxelGridPosition = this.spreadPointOnHeights(new Vector3fF(0,0,0));
	...
	private Vector3fF spreadPointOnHeights(Vector3fF position) {
	...
	public void spreadEntitiesOnHeights(Collection<IEntity> entityList) {
		if (!entityList.isEmpty()) {
			for (IEntity entity : entityList) {
				...
				entity.setPosition(new Vector3fF(entity.getPosition().x, terrainHeight, entity.getPosition().z));
			}
		}
	}
	...
	public void spreadParitclesOnHeights(Collection<ParticleSystem> systems) {
		if (!systems.isEmpty()) {
			for (ParticleSystem system : systems) {
				...
				system.setPosition(new Vector3fF(system.getPosition().x, terrainHeight, system.getPosition().z));
			}
		}
	}
